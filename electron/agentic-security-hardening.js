...
class AgenticSecurityHardening {
    ...
    async init() {
        // Initialize runtime hardeners (platform-specific fixes)
        const capabilities = await RuntimeHardener.initialize();
        
        // Use platform-detected staging path
        this.stagingPath = path.join(capabilities.stagingPath, this.stagingUUID);
        ...
        this.onBattery = capabilities.onBattery;
        ...
        console.log(`[Security] 🏰 Runtime hardeners: ACTIVE`);
        console.log(`[Security] 📁 Staging path: ${this.stagingPath}`);
        console.log(`[Security] 🔋 Battery status: ${this.onBattery ? 'ON BATTERY' : 'AC POWER'}`);
    }
    ...
    /**
     * Get CPU usage for process (FIXED: Uses accurate platform-specific methods)
     */
    async getProcessCPU(pid) {
        try {
            // Use hardened CPU monitor (fixes Windows wmic issue)
            return await CPUMonitorHardener.getProcessCPU(pid);
        } catch (error) {
            console.warn(`[Security] ⚠️ CPU monitoring failed for PID ${pid}: ${error.message}`);
            return 0;
        }
    }
    ...
    /**
     * Strip quarantine attributes from compiled binary (HARDENED)
     */
    async stripQuarantine(filepath) {
        try {
            // Use RuntimeHardener for proper platform-specific handling
            await RuntimeHardener.hardenBinary(filepath);
            console.log(`[Security] ✅ Binary hardened: ${filepath}`);
        } catch (error) {
            console.warn(`[Security] ⚠️ Binary hardening failed: ${error.message}`);
            // Non-fatal - binary might still run
        }
    }
    ...
    /**
     * Scrub secrets from text before logging (HARDENED: Prevents regex DoS)
     */
    scrubSecrets(text) {
        // Use chunked scrubbing to prevent regex DoS on large logs
        return RegexDoSHardener.scrubSecretsChunked(text, 4096);
    }
