...
export function activate(context: vscode.ExtensionContext) {
    ...
    console.log('🚀 Starting integrated BigDaddyG Model Server...');
    const server = serverApp.listen(SERVER_PORT, 'localhost', () => {
        console.log(`✅ BigDaddyG Model Server running on http://localhost:${SERVER_PORT}`);
    ...
    // Graceful shutdown
    process.on('SIGINT', () => {
        console.log('\n🛑 BigDaddyG Servers shutting down...');
        server.close();
        webIdeServer.close();
        process.exit(0);
    });
