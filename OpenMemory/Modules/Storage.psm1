...
$script:Users   = @{}
$script:Vectors = New-Object System.Collections.Generic.List[object]
$script:StoreRoot = ""
...
function Initialize-OMStorage {
    ...
    if (Test-Path "$Root\memories.jsonl") {
        Get-Content "$Root\memories.jsonl" -ErrorAction SilentlyContinue | ForEach-Object {
            try {
                $script:Memories.Add($_ | ConvertFrom-Json)
            } catch {
    ...
    if (Test-Path "$Root\embeddings.jsonl") {
        Get-Content "$Root\embeddings.jsonl" -ErrorAction SilentlyContinue | ForEach-Object {
            try {
                $script:Vectors.Add($_ | ConvertFrom-Json)
            } catch {
...
function Add-OMMemory {
    ...
    $script:Memories.Add($memory)
    
    # Generate and store embedding
    $embedding = Get-OMEmbedding $Content
    $script:Vectors.Add([ordered]@{
        id     = $id
        sector = $Sector
        vector = $embedding
    })
...
function Remove-OMMemory {
    ...
    foreach ($m in $script:Memories) {
        if ($m.id -eq $Id) {
            $script:Memories.RemoveAt($index)
            $found = $true
            break
        }
        $index++
    ...
    if ($found) {
        ...
        $vecIndex = 0
        foreach ($v in $script:Vectors) {
            if ($v.id -eq $Id) {
                $script:Vectors.RemoveAt($vecIndex)
                break
            }
            $vecIndex++
        }
        ...
        Write-Warning "[OpenMemory] ⚠️ Memory not found: $Id"
    }
}
...
function Clear-OMStorage {
    ...
    if (-not $Force) {
        Write-Warning "[OpenMemory] ⚠️ Use -Force to confirm clearing all memories"
        return
    }
    
    $script:Memories.Clear()
    $script:Vectors.Clear()
    $script:Users.Clear()
